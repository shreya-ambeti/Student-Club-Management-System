...
public class RemoveStudent extends JFrame implements ActionListener {
    ...
    RemoveStudent() {
        ...
        labelstId.setBounds(50, 50, 100, 30);
        add(labelstId);
        
        cstId = new Choice();
        cstId.setBounds(200, 50, 150, 30);
        add(cstId);
        
        try {
            Conn c = new Conn();
            String query = "select * from studentclub";
            ResultSet rs = c.s.executeQuery(query);
            while(rs.next()) {
                cstId.add(rs.getString("stId"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            Conn c = new Conn();
            String query = "select * from studentclub where stId = '"+cstId.getSelectedItem()+"'";
            ResultSet rs = c.s.executeQuery(query);
            while(rs.next()) {
        ...
        cstId.addItemListener(new ItemListener() {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == delete) {
            try {
                Conn c = new Conn();
                String query = "delete from studentclub where stId = '"+cstId.getSelectedItem()+"'";
